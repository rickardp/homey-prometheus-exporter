...
export default class SystemSource implements MetricSource {
  ...
  initialize = async (
    ...
    let systemInfo = (await api.system.getInfo()) as any; // Bad type info
  ...
  async updateSystemInfoMemory() {
    ...
    await this.profiling.timeAsyncCode(async () => {
      try {
        let memoryInfo = (await this.api.system.getMemoryInfo()) as any; // Bad types from Homey API
        ...
        for (let app in memoryInfo.types) {
          this.metrics.gauge_memory_used
            .labels(app)
            .set(memoryInfo.types[app].size);
        }
      } catch (err: any) {
  ...
  async updateSystemInfoCpu() {
    ...
    await this.profiling.timeAsyncCode(async () => {
      try {
        let systemInfo = (await this.api.system.getInfo()) as any;
        ...
        for (let time in systemInfo.cpus[0].times) {
          this.metrics.gauge_cpu
            .labels(time)
            .set(systemInfo.cpus[0].times[time]);
        }
      } catch (err: any) {
  ...
  async updateSystemInfoStorage() {
    ...
    await this.profiling.timeAsyncCode(async () => {
      try {
        let storageInfo = (await this.api.system.getStorageInfo()) as any;
        ...
        for (let app in storageInfo.types) {
          this.metrics.gauge_storage_used
            .labels(app)
            .set(storageInfo.types[app].size);
        }
      } catch (err: any) {
