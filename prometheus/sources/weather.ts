...
export default class WeatherSource implements MetricSource {
  ...
  updateWeather = (weather: any) => {
    this.profiling.timeCode(() => {
      if (weather) {
        this.metrics.gauge_weather_humidity.labels(weather.city, weather.country).set(weather.humidity * 100);
        this.metrics.gauge_weather_pressure.labels(weather.city, weather.country).set(weather.pressure * 1e3);
        this.metrics.gauge_weather_temperature.labels(weather.city, weather.country).set(weather.temperature);
      }
    }, 'weather');
  };
}
